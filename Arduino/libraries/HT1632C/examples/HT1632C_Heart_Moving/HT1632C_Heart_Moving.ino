...
void loop () {
  if (digitalRead(8) == LOW) {// Si notre bouton est en position LOW.
    TIME = 25;//On change le temps d'attente
  }

  if (digitalRead(8) == HIGH) {// Si notre bouton est en position HIGH.
    TIME = 100;//On change le temps d'attente
  }
  HT1632.renderTarget(0);// On envoie le tout à la matrice
  HT1632.clear();// On enlève l'affichage précédent.

  HT1632.selectChannel(0); // On choisit d'écrire en vert.
  HT1632.drawImage(IMG_HEART, IMG_HEART_WIDTH,  IMG_HEART_HEIGHT, j - IMG_HEART_WIDTH, 4);//On dessine notre coeur.

  HT1632.selectChannel(1); // On choisit d'écrire en rouge.
  HT1632.drawImage(IMG_HEART, IMG_HEART_WIDTH,  IMG_HEART_HEIGHT, OUT_SIZE * 2 - j, 4);//On dessine notre coeur.

  HT1632.render(); // On envoie le tout à la matrice

  HT1632.renderTarget(1);// On envoie le tout à la matrice
  HT1632.clear();  // On enlève l'affichage précédent.

  HT1632.selectChannel(0); // On choisit d'écrire en vert.
  HT1632.drawImage(IMG_HEART, IMG_HEART_WIDTH,  IMG_HEART_HEIGHT, j - IMG_HEART_WIDTH - OUT_SIZE, 4);//On dessine notre coeur.

  HT1632.selectChannel(1); // Green heart on second screen:
  HT1632.drawImage(IMG_HEART, IMG_HEART_WIDTH,  IMG_HEART_HEIGHT, OUT_SIZE - j, 4);//On dessine notre coeur.

  HT1632.render();// On envoie le tout à la matrice

  j = (j + 1) % (OUT_SIZE * 2); //On déplace notre notre image à la prochaine séquence
