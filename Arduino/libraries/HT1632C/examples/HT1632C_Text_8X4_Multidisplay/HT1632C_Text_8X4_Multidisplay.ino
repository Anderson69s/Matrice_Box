...
void loop () {
  if (digitalRead(8) == LOW) {// Si notre bouton est en position LOW.
    ...
    char disp [] = "Hello World !!!"; //On choissit notre texte
    wd = HT1632.getTextWidth(disp, FONT_8X4_END, FONT_8X4_HEIGHT);//On attache notre texte à notre affichage
    HT1632.renderTarget(0);// On envoie le tout à la matrice
    HT1632.clear();// On enlève l'affichage précédent.
    HT1632.drawText(disp, OUT_SIZE * 2 - i, 4, FONT_8X4, FONT_8X4_END, FONT_8X4_HEIGHT);// On affiche notre texte.
  ...
  if (digitalRead(8) == HIGH) {// Si notre bouton est en position HIGH.
    ...
    char disp [] = "How Are You ???";//On choissit notre texte
    wd = HT1632.getTextWidth(disp, FONT_8X4_END, FONT_8X4_HEIGHT);//On attache notre texte à notre affichage
    HT1632.renderTarget(0);// On envoie le tout à la matrice
    HT1632.clear();// On enlève l'affichage précédent.
    HT1632.drawText(disp, OUT_SIZE - i, 4, FONT_8X4, FONT_8X4_END, FONT_8X4_HEIGHT);// On affiche notre texte.
    HT1632.render();// On envoie le tout à la matrice
  }
  i = (i + 1) % (wd + OUT_SIZE * 2);//On fait avancer notre texte 
